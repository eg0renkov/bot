...
async def calendar_sync_reminders_callback(callback: CallbackQuery):
    """Синхронизация с напоминаниями - перенаправление на полную реализацию"""
    # Импортируем handler из yandex_integration
    from handlers.yandex_integration import calendar_sync_reminders_handler
    
    # Вызываем полную реализацию
    await calendar_sync_reminders_handler(callback)
